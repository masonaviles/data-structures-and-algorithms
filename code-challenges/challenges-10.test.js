...
const count = (target, input) => {
  return input.reduce((accumulator, currentValue) => {
    const onlyFives = currentValue.filter((value) => {
      return value === target;
    });
    const numberOfFives = onlyFives.length;
...
const totalSum = (input) => {
  return input.reduce((accumulator, currentValue) => {
    const sumOfValues = currentValue.reduce((accumulator2, currentValue2) => {
      return accumulator2 += currentValue2;
    }, 0);
    return accumulator += sumOfValues;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arrayOfNumbers => {
...
let findMaleAndFemale = (data) => {
  return data.filter(humans => {
    return humans.gender === 'female' || humans.gender === 'male';
  }).map(character => {
