...
// filter example
// const students = ['Audrey', 'Jake', 'Mason', 'Brian', 'Daniel', 'Jacok', 'Jessi', 'Jeremy', 'Julien'];
// let filteredStudents = students.filter((student, index, array) => {
...
function returnTen(str){
  const arr = str.split('', str.length);
  const newArr = arr.slice(str.length - 10, str.length);
  return newArr;
}
...
const typeNum = (arr) => {
  let filter = arr.filter(numbersOnly);
  return filter;
};
...
const containsAnd = (arr) => {
  const filter = arr.filter(name => {
    return name.includes('and');
  })
  return filter;
}
...
const filterStringsWithVowels = (arr) => {
  let vow = arr.filter((item) => item % 2 === 1);
  return vow;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const diff = arr.filter(x => !forbiddenValues.includes(x));
  return diff;
};
